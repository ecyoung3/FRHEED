...
class FRHEED(QtWidgets.QMainWindow, form_class):
    ...
    def update_frame(self):
        global avg1, avg2, avg3, t0, t, p, x1, y1, x2, y2, a1, b1, a2, b2, c1, d1, c2, d2, inverted, tstart, samplenum, grower, sampletextset, cmap, background, backgroundset, scaled_w, scaled_h, runstopwatch, timing, timer_active
...
#            img = np.flipud(img)
            ...
            if scale == 0:
                scale = 1
            if scale != 0:
                self.scaled_w = scale * img_width
                self.scaled_h = scale * img_height
    ...
    def record(self):
        ...
        if recording and running:
            ...
            vidnum = int(vidnum) + 1
            out = cv2.VideoWriter(path+filename+'.avi', fourcc, 35.0, (int(self.scaled_w), int(self.scaled_h)), True)
    ...
    def stopwatch(self):
        ...
        if not runstopwatch:
            ...
            self.savedtime = "%.2f" % self.savedtime
    ...
    def changetime(self):
        ...
        self.start_time = str(self.hr+':'+self.minu+':'+self.sec)
