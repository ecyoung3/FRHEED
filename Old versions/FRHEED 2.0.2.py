...
cmap = FRHEEDcmap
#cmp.test_colormap(FRHEEDcmap) # display a sample of the FRHEEDcmap
...
def grab(width, height, fps, queue):
    global grower, running, recording, basepath, path, filename, exposure, isfile, samplenum, vidnum, out, fourcc, setout, cam, scaled_w, scaled_h, inverted, cam
    while running:
        frame = {}        
...
class FRHEED(QtWidgets.QMainWindow, form_class):
    ...
    def __init__(self, parent=None):
        ...
        self.highexposuretime = int(config['VG']['highexposure'])
        self.lowexposuretime = int(config['VG']['lowexposure'])
    ...
    def highexposure(self):
        ...
        config['VG']['highexposure'] = val
        self.highexposuretime = config['VG']['highexposure']
    ...
    def lowexposure(self):
        ...
        config['VG']['lowexposure'] = val
        self.lowexposuretime = config['VG']['lowexposure']
...
#            img = np.flipud(img)
    ...
    def capture_image(self):
        global isfile, imnum, running, samplenum, path, filename, background, backgroundset, cmap, basepath
        if running:
            frame = q.get()
            img = frame["img"]
            if inverted:
...
camera_width = int(config['VG']['width'])
camera_height = int(config['VG']['height'])
fps = int(config['VG']['fps'])
capture_thread = threading.Thread(target=grab, args = (camera_width, camera_height, fps, q)) # RHEED camera is 2048 by 1536
...
#### TODO: ####
"""
- Fix 'All-NaN slice encountered b = (np.nanmin(d), np.nanmax(d))
- more advanced rectangle moving/resizing
